...
import { buyStock, sellStock } from '../services/api'; // Import API functions
...
const TradingChart = ({ onTradeSuccess }) => {
  ...
  // TRADING STATE
  const [quantity, setQuantity] = useState(0.1); // Default trade amount
  ...
  useEffect(() => {
    ...
    chart.subscribeClick((param) => {
      ...
      if (!firstPointRef.current) {
        ...
        if (p1.time === p2.time) return; // Prevent crash
  ...
  const handleTrade = async (type) => {
    ...
    try {
      if (type === 'BUY') {
        ...
        alert(`Successfully Bought ${quantity} ${symbol}`);
      } else {
    ...
    <div style={{ padding: '20px', background: '#131722', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
      ...
      <div style={{ marginTop: '20px', display: 'flex', gap: '10px', alignItems: 'center' }}>
        ...
        <button 
          ...
          disabled={isLoading}
          style={{ flex: 1, padding: '15px', background: isLoading ? '#555' : '#26a69a', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: isLoading ? 'not-allowed' : 'pointer' }}>
        ...
        <button 
          ...
          disabled={isLoading}
          style={{ flex: 1, padding: '15px', background: isLoading ? '#555' : '#ef5350', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: isLoading ? 'not-allowed' : 'pointer' }}>
